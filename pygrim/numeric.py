...
"""
# 6ae250 --- parse variables???
# todo: .is_integer() -> enable simplification!

from fungrim import *
from flint import *
from fungrim.numeric import *

eval = ArbNumericalEvaluation(cache={})
eval.eval(JacobiTheta(4, Div(1,4), ConstI, 1), wp=ctx.prec)

import fungrim.formulas
from mpmath import timing

def all_traverse():
    for entry in all_entries:
        try:
            eval.eval(entry, full_traversal=True, wp=ctx.prec)
        except (ValueError, NotImplementedError, ArbFiniteError):
            pass

all_traverse()
print len(eval.cache)


for expr in subexpressions(fungrim.formulas.entries_dict["6ae250"]):
    print expr
    try:
        print expr.n()
    except:
...
from flint import *
from fungrim.numeric import *
Where(1/x, Equal(x, ConstPi)).n()


(2*ConstPi*ConstI).n()

(Expr(-5) ** Expr(Div(1,2))).n()


Sum(1/Factorial(n), Tuple(n, 0, Infinity))

Sum(Fun(n, 1/Factorial(n)), Range(0, Infinity))

Sum(1/Factorial(n), n, Range(0, Infinity))

Sum(1/Factorial(n), n, 0, Infinity)

Sum(1/Factorial(n), For(n), 0, Infinity)

Sum(1/Factorial(n), For(n, 0, Infinity))

Sum(1/Factorial(n+m), For(n, m), Range(0, Infinity))


Sum(1/Factorial(n), For(n, 0, Infinity))

Var(n, Sum(1/Factorial(n), LessEqual(0, n, Infinity) && Prime(n))


Sum(q**(n**2), For(n), And(Element(n, ZZ), Unequal(n, 0)))

Zeros(Sin(x), For(x), Element(x, RR))

Zeros(For(x, Element(x, RR)), Sin(x))

Sum(1/Factorial(n), ForElement(x, ZZGreaterEqual(0)))


With(n, Sum(1/Factorial(n), From(x, 0, Infinity)))
With(t, Integral(Exp(-t**2), From(t, -Infinity, Infinity)))
With(x, Zeros(Sin(x), Element(x, RR)))


Sum, 1/n, n, 0, Infinity)


Sum(Var(n), 1/Factorial(n), 0, Infinity)
Integral(Var(x), Exp(-x**2), -Infinity, Infinity)

Sum(Var(n), 0, Infinity, 1/Factorial(n))
Integral(Var(x), -Infinity, Infinity, Exp(-x**2))

Var(n, Sum(1/Factorial(n), 0, Infinity))



Integral(Exp(-x**2), For(x), Over(-Infinity, Infinity))


Set(For(f(x), x, Element(x, ZZ)))
Union(For(f(x), x, Element(x, ZZ)))
Intersection(For(f(x), x, Element(x, ZZ)))

All(f(x), Var(x), Element(x, ZZ))
Any(f(x), Var(x), Element(x, ZZ))
Minimum(f(x), Var(x), Element(x, ZZ))
Maximum(f(x), Var(x), Element(x, ZZ))
ArgMin(f(x), Var(x), Element(x, ZZ))
ArgMax(f(x), Var(x), Element(x, ZZ))
ArgMinUnique(f(x), Var(x), Element(x, ZZ))
ArgMaxUnique(f(x), Var(x), Element(x, ZZ))
Solutions(f(x), Var(x), Element(x, ZZ))
UniqueSolution(f(x), Var(x), Element(x, ZZ))
Supremum(f(x), Var(x), Element(x, ZZ))
Infimum(f(x), Var(x), Element(x, ZZ))

Integral(f(x), Var(x), Element(x, RR))
Sum(f(x), Var(x), Element(x, ZZ))
Product(f(x), Var(x), Element(x, ZZ))



Entry(ID("08ff0b"),
    Formula(Equal(Totient(n), n - DivisorSum(Totient(d), Var(d), n, Less(d, n)))),
    Variables(n),
...
Where(1/(a+b), Equal(a, 2), Equal(b, 3))

Alias(Equal(a, 2), Equal(b, 3), 1/(a+b))



With(Alias(a, 2), Alias(b, 3), 1/(a+b))

With(Var(a, b), Equal(a, 2), Equal(b, 3), 1/(a+b))


Let(Equal(a, 2), Equal(b, 3)



With(Var(a, b), Equal(a, 2), Equal(b, 3), )
...
Limit(f(x), For(x), p, Element(x, ZZ))
