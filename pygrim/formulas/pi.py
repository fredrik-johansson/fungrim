...
make_entry(ID("f8d280"),
    Formula(Equal(Pi, 16*Acot(5) - 4*Acot(239))))
...
make_entry(ID("f617c0"),
    Formula(Equal(Pi, 4*Sum((-1)**k / (2*k+1), For(k, 0, Infinity)))))

make_entry(ID("fddfe6"),
    Formula(Equal(Pi, Sum((1 / 16**k) * (4/(8*k+1)-2/(8*k+4)-1/(8*k+5)-1/(8*k+6)), For(k, 0, Infinity)))),
...
make_entry(ID("69fe63"),
    Formula(Equal(Pi, 2*Product((4*k**2)/(4*k**2-1), For(k, 1, Infinity)))))

# Limit representations

make_entry(ID("e1e106"),
    Formula(Equal(Pi, SequenceLimit(16**k/(k*Binomial(2*k,k)**2), For(k, Infinity)))))
