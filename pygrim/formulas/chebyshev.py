...
make_entry(ID("7a7d1d"),
    Formula(Equal(Zeros(ChebyshevT(n,x), ForElement(x, CC)), Set(Cos(((2*k-1)/(2*n))*ConstPi), ForElement(k, ZZBetween(1, n))))),
    Variables(n),
    Assumptions(Element(n, ZZGreaterEqual(0))))

make_entry(ID("ce39ac"),
    Formula(Equal(Zeros(ChebyshevU(n,x), ForElement(x, CC)), Set(Cos((k/(n+1))*ConstPi), ForElement(k, ZZBetween(1, n))))),
    Variables(n),
    Assumptions(Element(n, ZZGreaterEqual(0))))

make_entry(ID("3d25dd"),
    Formula(Equal(Solutions(Brackets(Element(ChebyshevT(n,x), Set(-1,1))), ForElement(x, CC)), Set(Cos((k/n)*ConstPi), ForElement(k, ZZBetween(0, n))))),
    Variables(n),
    Assumptions(Element(n, ZZGreaterEqual(1))))

make_entry(ID("b5a25e"),
    Formula(Equal(Solutions(Brackets(Equal(ChebyshevT(n,x), 1)), ForElement(x, CC)), Set(Cos((2*k/n)*ConstPi), ForElement(k, ZZBetween(0, Floor(n/2)))))),
    Variables(n),
    Assumptions(Element(n, ZZGreaterEqual(1))))

make_entry(ID("db2b0a"),
    Formula(Equal(Solutions(Brackets(Equal(ChebyshevT(n,x), -1)), ForElement(x, CC)), Set(Cos(((2*k-1)/n)*ConstPi), ForElement(k, ZZBetween(1, Floor((n+1)/2)))))),
    Variables(n),
    Assumptions(Element(n, ZZGreaterEqual(1))))
