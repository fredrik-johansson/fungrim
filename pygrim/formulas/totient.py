...
make_entry(ID("cd7877"),
    Formula(Equal(SequenceLimitSuperior(Totient(n) / n, n, Infinity), 1)))

make_entry(ID("acfc1f"),
    Formula(Equal(SequenceLimitInferior((Totient(n) * Log(Log(n))) / n, n, Infinity), Exp(-ConstGamma))))

make_entry(ID("4b5b44"),
    Formula(Equal(Limit(Totient(n) / n**(1-delta), n, Infinity), Infinity)),
    Variables(delta),
    Assumptions(Element(delta, OpenInterval(0, Infinity))))

make_entry(ID("33139b"),
    Formula(Equal(SequenceLimitInferior(Totient(n+1) / Totient(n), n, Infinity), 0)))

make_entry(ID("feb1a0"),
    Formula(Equal(SequenceLimitSuperior(Totient(n+1) / Totient(n), n, Infinity), Infinity)))

make_entry(ID("8d7b3d"),
    Formula(Equal(SequenceLimit(1/N**2 * Sum(Totient(n), Tuple(n, 1, N)), N, Infinity), 3/ConstPi**2)))

make_entry(ID("9923b7"),
    Formula(Equal(SequenceLimit(1/Log(N) * Sum(1/Totient(n), Tuple(n, 1, N)), N, Infinity), (RiemannZeta(2)*RiemannZeta(3) / RiemannZeta(6)))))
